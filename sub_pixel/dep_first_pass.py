...
#convert to greyscale immediately
img_to_convert = cv2.imread(image_path, cv2.IMREAD_GRAYSCALE)
# img_to_convert = cv2.equalizeHist(img_to_convert)
cv2.imshow("original", img_to_convert)
cv2.waitKey(0)
...
class Ramp: 
    ...
    def cache_ramp_maps(self):
        ...
        for char in self.ramp_string:
            ...
            self.brightness_dict_flat[char] = np.array(char_map_pil)
    ...
    def get_char_region_subpixel(self, region):
        ...
        #     difference = np.sum((region - char_arr) ** 2)
        ...
        for char in self.brightness_dict_flat:
            ...
            for y in range(len(region)):
                for x in range(len(region[y])):
                    ...
                    this_pixel_diff = abs(color_avg_region - color_avg_char_arr)
...
ascii_image = Image.new("RGB", (i_width, i_height), "white")
